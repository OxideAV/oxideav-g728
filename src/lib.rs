...
/// Register the G.728 codec (decoder + encoder) with the supplied
/// [`CodecRegistry`]. Prefer the unified [`register`] entry point when
/// you have a [`oxideav_core::RuntimeContext`] in hand.
pub fn register_codecs(reg: &mut CodecRegistry) {
    codec::register_codecs(reg);
}

/// Unified registration entry point — installs G.728 into the codec
